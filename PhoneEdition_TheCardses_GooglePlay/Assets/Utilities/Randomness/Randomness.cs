...
//	Random shuffling of the supplied integer array.
	...
	public static void Shuffle<T> (System.Collections.Generic.List<T> list) {
		...
		for (int i = 0; i < list.Count; i++) {
			temp = list[i];
			int swapIndex = RndRange (list.Count);
...
//	Now Works with an int array too!
	public static int Sample(int[] distro) {
		float[] f_distro = new float[distro.Length];
		for (int i = 0; i < distro.Length; i++) {
			f_distro[i] = (float)distro[i];
		}

		float total = Sum(f_distro);
		return Sample(f_distro, total);
	}
